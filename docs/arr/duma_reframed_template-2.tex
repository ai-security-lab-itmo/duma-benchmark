...
\begin{document}
    ...
    \paragraph{mail\_rag\_phishing (RAG poisoning).} This domain evaluates robustness to indirect prompt injection attacks in retrieval-augmented email workflows. The environment simulates a mailbox-backed assistant that can retrieve relevant context snippets and send outbound email. Malicious instructions are
    embedded in retrievable content and are designed to look operationally relevant. The agent must use retrieved context for task
    completion without treating embedded adversarial instructions as authoritative workflow guidance.
